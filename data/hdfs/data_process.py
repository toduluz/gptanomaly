...
input_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../.dataset/hdfs'))
...
def hdfs_sampling(log_file, window='session'):
    ...
    data_list = [(key, values['EventTemplate'], values['EventId']) for key, values in data_dict.items()]
