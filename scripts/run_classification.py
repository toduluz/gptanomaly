...
def main():
    ...
    def preprocess_function(examples):
        ...
        inputs = [inp.split('|') for inp in examples["sentence"]]
        labels = examples["label"]
        for i in range(len(inputs)):
            seq_level_shuffle = False
            log_level_shuffle = False
            insert_seq = False
            delete_seq = False
            delete_word = False
            replace_seq = False
            ...
            if num < 0.1:
                seq_level_shuffle = True
            elif num < 0.2:
                log_level_shuffle = True
            elif num < 0.3:
                delete_seq = True
            elif num < 0.4:
                delete_word = True
            ...
            if seq_level_shuffle:
                random.shuffle(inputs[i])
                postfix = "Answer: It is an anomaly as the sequence of log is in shuffled order."
                inputs[i].append(postfix)
            elif log_level_shuffle:
                prob = np.random.uniform()
                num_log_shuffle = math.ceil(prob * len(inputs[i]))
                random_indices = np.random.choice(range(len(inputs[i])), size=num_log_shuffle, replace=False)
                for k in random_indices:
                    tmp = copy.deepcopy(inputs[i][k])
                    tmp = tmp.split(" ")
                    inputs[i][k] = " ".join(random.sample(tmp, len(tmp)))
                postfix = "Answer: It is an anomaly as the words in some of the log sentences are shuffled."
                inputs[i].append(postfix)
            elif delete_seq:
                prob = np.random.uniform()
                num_log_delete = math.ceil(prob * len(inputs[i]))
                random_indices = np.random.choice(range(len(inputs[i])), size=num_log_delete, replace=False)
                new_log = []
                for k in range(len(inputs[i])):
                    if k not in random_indices:
                        new_log.append(inputs[i][k])
                inputs[i] = new_log
                postfix = "Answer: It is an anomaly as some log sentences are deleted from the sequence of log."
                inputs[i].append(postfix)
            elif delete_word:
                for k in range(len(inputs[i])):
                    tmp = inputs[i][k]
                    tmp = tmp.split(" ")
                    prob = np.random.uniform()
                    num_word_delete = math.ceil(prob * len(tmp))
                    random_indices = np.random.choice(range(len(tmp)), size=num_word_delete, replace=False)
                    new_tmp = []
                    for j in range(len(tmp)):
                        if j not in random_indices:
                            new_tmp.append(tmp[j])
                    inputs[i][k] = " ".join(new_tmp)
                postfix = "Answer: It is an anomaly as some words are deleted from each log sentence."
                inputs[i].append(postfix)
            elif replace_seq:
                prob = np.random.uniform()
                num_log_replace = math.ceil(prob * len(inputs[i]))
                random_indices = np.random.choice(range(len(inputs[i])), size=num_log_replace, replace=False)
                replace_log_seq = np.random.choice(range(len(inputs)), size=num_log_replace, replace=False)
                replace_log_statement = [np.random.choice(range(len(inputs[seq])), replace=False) for seq in replace_log_seq]
                new_log = []
                counter = 0
                for k in range(len(inputs[i])):
                    if k not in random_indices:
                        ...
                        new_log.append(inputs[replace_log_seq[counter]][replace_log_statement[counter]])
                        counter += 1
                inputs[i] = new_log
                postfix = "Answer: It is an anomaly as some log sentences are replaced with log sentences from other log sequences."
                inputs[i].append(postfix)
            elif insert_seq:
                prob = np.random.uniform()
                num_log_insert = math.ceil(prob * len(inputs[i]))
                random_indices = np.random.choice(range(len(inputs[i])), size=num_log_insert, replace=False)
                insert_log_seq = np.random.choice(range(len(inputs)), size=num_log_insert, replace=False)
                insert_log_statement = [np.random.choice(range(len(inputs[seq])), replace=False) for seq in insert_log_seq]
                new_log = []
                counter = 0
                for k in range(len(inputs[i])):
                    if k not in random_indices:
                        ...
                        new_log.append(inputs[insert_log_seq[counter]][insert_log_statement[counter]])
                        new_log.append(inputs[i][k])
                        counter += 1
                inputs[i] = new_log
                postfix = "Answer: It is an anomaly as it has been inserted with a few random log sentences from other log sequences."
                inputs[i].append(postfix)
            else:
                postfix = "Answer: It is not an anomaly as the order of log sequence and the order of words in each log statement are preserved."
                inputs[i].append(postfix)

            if seq_level_shuffle or log_level_shuffle or delete_seq or delete_word or replace_seq or insert_seq:
                labels[i] = 1

        prefix = "Is the following sequence of log an anomaly? "
        examples["sentence"] = [prefix + " ".join(inp) for inp in inputs]
        # Tokenize the texts
        result = tokenizer(examples["sentence"], padding=padding, max_length=max_seq_length, truncation=True)
        if label_to_id is not None and "label" in examples:
            if is_multi_label:
                result["label"] = [multi_labels_to_ids(l) for l in labels]
            else:
                result["label"] = [(label_to_id[str(l)] if l != -1 else -1) for l in labels]
        return result
    ...
    def preprocess_function_eval(examples):
        ...
        prefix = "Is the following sequence of log an anomaly? "
        postfix = "Answer: "
        examples["sentence"] = [prefix + " ".join(inp) + postfix for inp in inputs]
        # Tokenize the texts
        result = tokenizer(examples["sentence"], padding=padding, max_length=max_seq_length, truncation=True)
        if label_to_id is not None and "label" in examples:
            if is_multi_label:
    ...
    if training_args.do_train:
        ...
        train_dataset_pre = raw_datasets["train"]
        with training_args.main_process_first(desc="dataset map pre-processing"):
    ...
    if training_args.do_eval:
        if "validation" not in raw_datasets and "validation_matched" not in raw_datasets:
            ...
            eval_dataset_pre = raw_datasets["validation"]
            with training_args.main_process_first(desc="dataset map pre-processing"):
                eval_dataset = eval_dataset_pre.map(
    ...
    if training_args.do_predict or data_args.test_file is not None:
        ...
        predict_dataset_pre = raw_datasets["test"]
        with training_args.main_process_first(desc="dataset map pre-processing"):
                predict_dataset = predict_dataset_pre.map(
                    preprocess_function_eval,
                    batched=True,
                    load_from_cache_file=not data_args.overwrite_cache,
                    desc="Running tokenizer on eval dataset",
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        if is_regression:
            ...
            p = precision.compute(predictions=preds, references=ref)
            recall = evaluate.load("recall")
            r = recall.compute(predictions=preds, references=ref)
            result = metric.compute(predictions=preds, references=ref)
        if len(result) > 1:
